...
class CheckIsCompletelyBinaryTree {
    ...
    /**
     * 使用BFS(广度优先遍历)判断是否是完全二叉树
     */
    public static boolean isCompletelyByBFS(Node root) {
        if (root == null) {
            return true;
        }
        // 是否遇到过左右两个孩子不双全的节点
        ...
        while (!queue.isEmpty()) {
            ...
            if (node.left == null && node.right != null) {
                // 节点只有右节点，没有左节点，那么一定不是完全二叉树
                return false;
            }
            if (flag) {
                // 遇到了左右节点不全的情况了，后续的节点必须是叶子节点，否则就不是完全二叉树
                if (!(node.left == null && node.right == null)) {
                    return false;
                }
            }
    ...
    private static Data recurse(Node subTreeRoot) {
        ...
        // 当前树是否是满二叉树
        boolean isFull = (leftData.isFull && rightData.isFull) && (leftData.high == rightData.high);
        // 当前的树高
    ...
    private static class Data {
        ...
        Data(int high, boolean isFull, boolean isCompletely) {
            this.high = high;
            this.isFull = isFull;
    ...
    private static class CannotHappenException extends RuntimeException {
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < testTimes; i++) {
            ...
            boolean r3 = isCompletelyBinaryTree(head);
            boolean r4 = isCompletelyByBFS(head);
