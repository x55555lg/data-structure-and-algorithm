...
class MaxPartyHappyValue {
    ...
    public static int getMaxHappyValue(Employee boss) {
        ...
        return Math.max(data.joinHappyValue, data.notJoinHappyValue);
    }
    ...
    private static Data recurse(Employee employee) {
        ...
        // 当前员工参与派对
        int joinHappyValue = employee.happyValue;
        // 当前员工不参与派对
        int notJoinHappyValue = 0;
        for (Employee subordinate : employee.subordinates) {
            // 当前员工的下属的快乐值信息
            Data subordinateData = recurse(subordinate);

            // 当前员工参与了，当前员工的下属不参与时的快乐值累加
            joinHappyValue += subordinateData.notJoinHappyValue;

            // 当前员工不参与了，当前员工的下属在参与，不参与情况下哪个大取哪个
            notJoinHappyValue += Math.max(subordinateData.joinHappyValue, subordinateData.notJoinHappyValue);
        }
        return new Data(joinHappyValue, notJoinHappyValue);
    }
    ...
    private static class Data {
        int joinHappyValue;
        int notJoinHappyValue;

        Data(int joinHappyValue, int notJoinHappyValue) {
            this.joinHappyValue = joinHappyValue;
            this.notJoinHappyValue = notJoinHappyValue;
        }
    }
