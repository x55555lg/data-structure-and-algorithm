package com.lg.datastructure.binarytree.binarytreerecurse.practice;

import java.util.ArrayList;
import java.util.HashMap;
import java.util.HashSet;

/**
 * 二叉树的递归套路练习
 *
 * @author Xulg
 * Created in 2020-11-02 10:09
 */
@SuppressWarnings("MapOrSetKeyShouldOverrideHashCodeEquals")
class FindTreeMaxDistance {

    /*
     * 给定一颗二叉树的头节点head，任何两个节点之间都存在距离，返回整颗二叉树的最大距离？
     * 两节点之间的距离：从节点1走到节点2需要走多少步
     * 例如如下的二叉树：
     *		            ①
     *	            ╱     ╲
     *	         ╱	          ╲
     *         ②	             ③
     *      ╱	  ╲	       ╱  ╲
     *   ╱	         ╲     ╱	      ╲
     * ④			  ⑤  ⑥		    ⑦
     * 节点④到节点⑤的距离：④ ② ⑤ 距离为3
     * 节点④到节点⑤的距离：④ ② ① ③ ⑦ 距离为5
     *------------------------------------------------------------------------------------
     * 思路：从head头节点X开始
     *  最大距离和X节点无关，最大距离 = max(左子树最大距离，右子树最大距离);
     *  最大距离和X节点有关，最大距离 = 左子树高度 + 1 + 右子树高度;
     */

    /**
     * 获取二叉树的最大距离
     */
    public static int getMaxDistance(Node root) {
        if (root == null) {
            return 0;
        }
        return calcMaxDistance(root).maxDistance;
    }

    private static Info calcMaxDistance(Node node) {
        if (node == null) {
            return new Info(0, 0);
        }

        // 左子树的最大距离和高度
        Info leftInfo = calcMaxDistance(node.left);
        // 右子树的最大距离和高度
        Info rightInfo = calcMaxDistance(node.right);

        // 当前树的高度
        int currentTreeHeight = Math.max(leftInfo.height, rightInfo.height) + 1;
        // 最大距离和头节点无关，取左右子树中最大的那个距离
        int leftRightSubTreeMaxDistance = Math.max(leftInfo.maxDistance, rightInfo.maxDistance);
        // 最大距离和头节点有关，当前树的最大距离 = 左树高度 + 1 + 右树高度
        int currentTreeMaxDistance = leftInfo.height + 1 + rightInfo.height;
        // 看哪个距离大就取哪个
        int maxDistance = Math.max(leftRightSubTreeMaxDistance, currentTreeMaxDistance);
        return new Info(maxDistance, currentTreeHeight);
    }

    /* 对数器 start */

    /**
     * 暴力方法获取最大距离
     */
    public static int getMaxDistanceForCompare(Node head) {
        if (head == null) {
            return 0;
        }
        ArrayList<Node> arr = getPreList(head);
        HashMap<Node, Node> parentMap = getParentMap(head);
        int max = 0;
        for (int i = 0; i < arr.size(); i++) {
            for (int j = i; j < arr.size(); j++) {
                max = Math.max(max, distance(parentMap, arr.get(i), arr.get(j)));
            }
        }
        return max;
    }

    private static ArrayList<Node> getPreList(Node head) {
        ArrayList<Node> arr = new ArrayList<>();
        fillPreList(head, arr);
        return arr;
    }

    private static void fillPreList(Node head, ArrayList<Node> arr) {
        if (head == null) {
            return;
        }
        arr.add(head);
        fillPreList(head.left, arr);
        fillPreList(head.right, arr);
    }

    private static HashMap<Node, Node> getParentMap(Node head) {
        HashMap<Node, Node> map = new HashMap<>();
        map.put(head, null);
        fillParentMap(head, map);
        return map;
    }

    private static Node generateRandomBST(int maxLevel, int maxValue) {
        return generate(1, maxLevel, maxValue);
    }

    private static void fillParentMap(Node head, HashMap<Node, Node> parentMap) {
        if (head.left != null) {
            parentMap.put(head.left, head);
            fillParentMap(head.left, parentMap);
        }
        if (head.right != null) {
            parentMap.put(head.right, head);
            fillParentMap(head.right, parentMap);
        }
    }

    private static int distance(HashMap<Node, Node> parentMap, Node o1, Node o2) {
        HashSet<Node> o1Set = new HashSet<>();
        Node cur = o1;
        o1Set.add(cur);
        while (parentMap.get(cur) != null) {
            cur = parentMap.get(cur);
            o1Set.add(cur);
        }
        cur = o2;
        while (!o1Set.contains(cur)) {
            cur = parentMap.get(cur);
        }
        Node lowestAncestor = cur;
        cur = o1;
        int distance1 = 1;
        while (cur != lowestAncestor) {
            cur = parentMap.get(cur);
            distance1++;
        }
        cur = o2;
        int distance2 = 1;
        while (cur != lowestAncestor) {
            cur = parentMap.get(cur);
            distance2++;
        }
        return distance1 + distance2 - 1;
    }

    private static Node generate(int level, int maxLevel, int maxValue) {
        if (level > maxLevel || Math.random() < 0.5) {
            return null;
        }
        Node head = new Node((int) (Math.random() * maxValue));
        head.left = generate(level + 1, maxLevel, maxValue);
        head.right = generate(level + 1, maxLevel, maxValue);
        return head;
    }

    /* 对数器 end */

    /* ************************************************************************************************************** */

    private static class Node {
        int value;
        Node left;
        Node right;

        Node(int value) {
            this.value = value;
        }
    }

    private static class Info {
        int maxDistance;
        int height;

        Info(int dis, int h) {
            maxDistance = dis;
            height = h;
        }
    }

    /* ************************************************************************************************************** */

    public static void main(String[] args) {
        int maxLevel = 4;
        int maxValue = 100;
        int testTimes = 1000000;
        for (int i = 0; i < testTimes; i++) {
            Node head = generateRandomBST(maxLevel, maxValue);
            if (getMaxDistance(head) != getMaxDistanceForCompare(head)) {
                System.out.println("Oops!");
            }
        }
        System.out.println("finish!");
    }
}
