...
class DescendPair {
    ...
    private static class Best {
        public static int calculateArrayDescendPairCount(int[] arr) {
        ...
        private static int recurse(int[] arr, int left, int right) {
            ...
            int leftCount = recurse(arr, left, mid);
            int rightCount = recurse(arr, mid + 1, right);
            int curCount = merge(arr, left, right, mid);
        ...
        private static int merge(int[] arr, int left, int right, int mid) {
            ...
            while (leftIdx <= mid && rightIdx <= right) {
                if (arr[leftIdx] > arr[rightIdx]) {
                    /*
                     *产生逆序对
                     * 在[leftIdx, mid]上有这么N个数大于arr[rightIdx]
                     */
    ...
    public static void main(String[] args) {
        ...
        for (int time = 0; time < times; time++) {
            ...
            int c1 = BruteForce.calculateArrayDescendPairCount(arr.clone());
            int c2 = Best.calculateArrayDescendPairCount(arr.clone());
